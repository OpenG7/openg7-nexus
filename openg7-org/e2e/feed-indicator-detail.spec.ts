...
test.describe('Feed indicator detail', () => {
  test('persists an indicator subscription created from the subscribe CTA across reloads', async ({
    page,
  }) => {
  ...
  test('shows a success toast when creating an alert from the legacy indicator route', async ({
    page,
  }) => {
    await mockAuthenticatedSessionApis(page);
    await seedAuthenticatedSession(page);
    ...
    await page
      .locator('[data-og7="indicator-alert-drawer"] textarea')
      .fill('Notify the manufacturing coordination team if the backlog index rises further.');
    await page.locator('[data-og7-id="indicator-alert-submit"]').click();
