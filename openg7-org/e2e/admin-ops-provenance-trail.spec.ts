...
test.describe('Admin ops provenance trail', () => {
  test('updates visible provenance metadata when a refresh succeeds', async ({ page }) => {
    ...
    const securityEntry = page.locator(
      '[data-og7="admin-ops-provenance-entry"][data-og7-id="security"]',
    );
    await expect(trail).toBeVisible();
    await expect(page.locator('[data-og7-id="admin-ops-provenance-status"]')).toHaveAttribute(
      'data-og7-state',
      'fresh',
    );
    await expect(securityEntry).toHaveAttribute('data-og7-state', 'fresh');
    await expect(securityEntry.locator('[data-og7="admin-ops-provenance-source"]')).toContainText(
  ...
  test('marks provenance as preserved-last-good when a refresh fails after a valid snapshot', async ({
    ...
    await expect(page.locator('[data-og7-id="admin-ops-provenance-status"]')).toContainText(
