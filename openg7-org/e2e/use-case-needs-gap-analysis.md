...
| Mise en relation | Initier un linkup, consulter l'historique, ouvrir le detail, suivre le statut, tracer la relation | partiel | oui | partiel | partiel | Le MVP prouve s'arrete a `historique -> detail -> note interne -> suivi manuel du statut -> persistence retour historique` sur un linkup existant. La creation directe reste hors MVP; acceptation/refus, messagerie, pieces jointes et journal formel correspondent a une prochaine branche produit, donc leur preuve E2E est intentionnellement differee | moyenne |
...
| Trust et validation | Moderer, verifier, historiser les decisions de confiance et rendre le statut lisible | oui | oui | oui | oui | `admin/trust` et `admin-trust-visibility.spec.ts` prouvent maintenant `review queue -> correction request -> rejection -> visibilite publique sur /partners/:id`. Les enrichissements futurs relevent plutot d'une profondeur operationnelle supplementaire que d'un manque de workflow prouve | moyenne |
...
3. Keep direct linkup creation out of the E2E backlog until the product scope changes, because `docs/frontend/linkup-functional-analysis.md` constrains the current MVP to existing-linkup history, detail, notes, and manual status follow-up; if scope expands, start with one explicit accept/refuse branch before messaging or attachments.
