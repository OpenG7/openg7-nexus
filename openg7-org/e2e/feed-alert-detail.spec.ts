...
test.describe('Feed alert detail', () => {
  test('redirects anonymous users to login when subscribing from alert detail', async ({
    page,
  }) => {
  ...
  test('renders alert details with report flow and related links', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-og7="alert-update-field"][data-og7-id="summary"]'),
    ).toBeVisible();
  ...
  test('shows an error toast when linked opportunity navigation fails', async ({ page }) => {
    ...
    await expect(page.locator('[data-og7="alert-detail-page"]')).toBeVisible();
    await expect(
      page.locator('[data-og7="notification-toast"][data-og7-id="error"]').filter({
        hasText:
