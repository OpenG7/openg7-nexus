...
| Partially implemented workflow | Linkup workflow depth | `partial` | current MVP proof stops at `history -> detail -> internal note -> manual status update -> persistence back into history` on an existing linkup. Direct creation remains outside current MVP; acceptance/refusal, messaging, attachments, and formal relation journaling are intentionally deferred until a next product branch exposes them visibly |
...
- Linkup proof should stay anchored to the existing-linkup MVP documented in [`../docs/frontend/linkup-functional-analysis.md`](../docs/frontend/linkup-functional-analysis.md): history, detail, internal note, manual status follow-up, and persistence back into history.
- `e2e/linkup-workflow.spec.ts` and `src/app/domains/matchmaking/pages/linkup-detail/og7-linkup-detail-page.component.spec.ts` already preserve that MVP slice. Direct creation, accept/refuse branches, messaging, attachments, and formal relation journaling should remain out of the proof backlog until the product scope expands.
...
| Linkup workflow depth | Partially implemented workflow | `yes` | Hold the current proof line until the product scope expands beyond the existing-linkup MVP; if it does, the next branch should be one explicit accept/refuse decision before any messaging or attachment handling | `P3` |
...
| 5 | `e2e/linkup-workflow-branch-depth.spec.ts` | one explicit accept-or-refuse decision on an existing linkup once that branch is intentionally exposed in detail, history, and timeline | linkup depth advances only after the product exposes one concrete branching decision beyond the current note-and-status MVP | `yes` | `Frontend + E2E` |

Operationally, only the fourth is still proof-first. The first, second, third, and fifth are product-following proofs and should wait for visible product closure; for linkups specifically, the current MVP proof is already sufficient until one explicit accept/refuse branch is exposed.
