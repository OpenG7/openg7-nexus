...
const highlightedHydrocarbonItem = {
...
const adjacentHydrocarbonItem = {
  id: 'hydrocarbon-opportunity-002',
  createdAt: '2026-03-26T08:30:00.000Z',
  updatedAt: '2026-03-26T10:15:00.000Z',
  type: 'OFFER',
  sectorId: 'energy',
  title: 'Western storage release for Ontario refiners',
  summary: 'Prairie Basin Logistics can release an 18-day crude buffer for Ontario refiners.',
  fromProvinceId: 'ab',
  toProvinceId: 'on',
  mode: 'EXPORT',
  quantity: {
  ...
  urgency: 2,
  credibility: 3,
  tags: ['hydrocarbon', 'storage'],
  source: {
    kind: 'COMPANY',
    label: 'Prairie Basin Logistics',
  },
  metadata: {
    publicationForm: {
      formKey: 'hydrocarbon-surplus-offer',
    ...
    extensions: {
      companyName: 'Prairie Basin Logistics',
      publicationType: 'inventory-release',
      volumeBarrels: 32000,
    },
  },
} as const;

test.describe('Feed decision path depth', () => {
  test('keeps route view, inherited context, and one explicit refinement legible through detail, unwind, and reload', async ({
    page,
  }) => {
...
async function mockHydrocarbonDecisionPathApis(page: Page): Promise<{
  ...
  const itemsById = new Map([
    [highlightedHydrocarbonItem.id, highlightedHydrocarbonItem],
  ...
  await page.route(/\/api\/feed\/[^/?]+(?:\?.*)?$/i, async route => {
    ...
    const item = itemsById.get(itemId) ?? highlightedHydrocarbonItem;
