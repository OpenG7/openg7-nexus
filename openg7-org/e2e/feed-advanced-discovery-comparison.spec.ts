...
async function expectSelectedLabel(
  page: Page,
  selector: string,
...
test.describe('Feed advanced discovery comparison', () => {
  test('keeps comparison-oriented discovery state coherent across two filter combinations and a detail roundtrip', async ({
    page,
  }) => {
    ...
    await expect(page.locator('[data-feed-item-id="request-002"]')).toContainText(
    ...
    await expect(page.locator('[data-feed-item-id="request-003"]')).toContainText(
