...
export default ({ strapi }: { strapi: Core.Strapi }) => ({
  async flows(ctx: Context) {
    const payload = await strapi
      ...
      .getFlows((ctx.query ?? {}) as ImportationQuery);
  ...
  async commodities(ctx: Context) {
    const payload = await strapi
      .service('api::importation.importation')
      .getCommodities((ctx.query ?? {}) as ImportationQuery);
  ...
  async riskFlags(ctx: Context) {
    const payload = await strapi
      .service('api::importation.importation')
      .getRiskFlags((ctx.query ?? {}) as ImportationQuery);
    ctx.set('cache-control', 'public, max-age=300');
    ctx.body = payload;
  },
  ...
  async suppliers(ctx: Context) {
    const payload = await strapi
      .service('api::importation.importation')
      .getSuppliers((ctx.query ?? {}) as ImportationQuery);
    ctx.set('cache-control', 'public, max-age=300');
    ctx.body = payload;
  },
  ...
  async knowledge(ctx: Context) {
    const payload = await strapi
      .service('api::importation.importation')
      .getKnowledge((ctx.query ?? {}) as ImportationQuery);
  ...
  async createWatchlist(ctx: Context) {
    try {
      const payload = await strapi
        .service('api::importation.importation')
        .createWatchlist((ctx.request.body ?? {}) as { name?: unknown; filters?: unknown });
      ctx.status = 201;
      ctx.body = payload;
    } catch (error) {
    ...
    try {
      const payload = await strapi
        ...
        .updateWatchlist(
          ...
          (ctx.request.body ?? {}) as { name?: unknown; filters?: unknown },
        );
      ctx.status = 200;
      ctx.body = payload;
    } catch (error) {
  ...
  async scheduleReport(ctx: Context) {
    try {
      const payload = await strapi
        .service('api::importation.importation')
        .scheduleReport((ctx.request.body ?? {}) as ImportationQuery);
      ctx.status = 202;
      ctx.body = payload;
    } catch (error) {
