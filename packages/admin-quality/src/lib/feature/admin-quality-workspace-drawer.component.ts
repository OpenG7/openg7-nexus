...
} from '../pages/admin-quality-action-registry';
import {
  AdminQualityDelegationPlan,
  AdminQualityDelegationMode,
} from '../pages/admin-quality-delegation';
...
const WORKSPACE_SURFACES: readonly AdminQualityWorkspaceSurface[] = [
  'qaQueue',
  'delegation',
  'actions',
];
    ...
    return [
      ...
      },
      {
        label: this.translate.instant(
          'admin.quality.workspace.recalculation.factual.repoSignalCommit',
        ),
        value: entry.factualSignals.repoSignalCommit ?? 'n/a',
      },
      {
        label: this.translate.instant(
          'admin.quality.workspace.recalculation.factual.repoSignalSource',
        ),
        value: entry.factualSignals.repoSignalSource ?? 'n/a',
      },
      {
        label: this.translate.instant(
          'admin.quality.workspace.recalculation.factual.latestDecisionAt',
        ),
  ...
  protected openGitHubIssue(plan: AdminQualityDelegationPlan): void {
    ...
    if (!url) {
      this.notifications.info(
        this.translate.instant('admin.quality.workspace.notifications.githubUnavailable'),
        {
    ...
    try {
      ...
      try {
        ...
        this.notifications.error(
          this.translate.instant('admin.quality.workspace.notifications.copyFailed'),
          {
            source: 'admin-quality',
          },
        );
